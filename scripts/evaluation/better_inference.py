...
def get_latent_z_with_hidden_states(model, videos):
    b, c, t, h, w = videos.shape
    x = rearrange(videos, 'b c t h w -> (b t) c h w')
    x = torch.nn.functional.interpolate(x, scale_factor=0.95, mode='bilinear', align_corners=False)
...
def image_guided_synthesis(model, prompts, videos, noise_shape, ddim_steps=50, ddim_eta=1., \
    ...
    if model.model.conditioning_key == 'hybrid':
        ...
        target_spatial = noise_shape[2:]  # [T, H, W]
        _, _, t_c, h_c, w_c = img_cat_cond.shape
        _, _, t_n, h_n, w_n = noise_shape

        # Make sure temporal dim is already correct
        assert t_c == t_n, f"Temporal mismatch: {t_c} vs {t_n}"

        if h_c != h_n or w_c != w_n:
            print(f"[SPATIAL FIX] Resizing c_concat from ({h_c}, {w_c}) to ({h_n}, {w_n})")
            img_cat_cond = torch.nn.functional.interpolate(
                img_cat_cond,
                size=(t_n, h_n, w_n),  # ✅ full 3D spatial size
                mode="trilinear",
                align_corners=False
            )
    ...
    #     if img_cat_cond.shape[1] != noise_shape[1]:
    #         repeat_factor = noise_shape[1] // img_cat_cond.shape[1]
    #         if noise_shape[1] % img_cat_cond.shape[1] != 0:
    #             raise ValueError(f"Cannot match channels: {img_cat_cond.shape[1]} -> {noise_shape[1]}")
    #         img_cat_cond = img_cat_cond.repeat(1, repeat_factor, 1, 1, 1)
    #
    #     cond["c_concat"] = [img_cat_cond]
    ...
    if unconditional_guidance_scale != 1.0:
        ...
        if model.model.conditioning_key == 'hybrid':
            uc_img_cat_cond = img_cat_cond.clone()
            uc["c_concat"] = [uc_img_cat_cond]
    ...
    print("[DEBUG] About to call ddim_sampler.sample()")
    print(f"Shape: {noise_shape}, dtype: {model.dtype}")
    ...
    # Replace intermediate frames
    index = list(range(samples.shape[2]))
    if len(index) > 2:  # avoid index errors for short sequences
        del index[1]
        del index[-2]
        samples = samples[:, :, index, :, :]
        batch_images_middle = model.decode_first_stage(samples, **additional_decode_kwargs)
        batch_images[:, :, batch_images.shape[2] // 2 - 1:batch_images.shape[2] // 2 + 1] = batch_images_middle[:, :,
                                                                                            batch_images.shape[
                                                                                                2] // 2 - 2:
                                                                                            batch_images.shape[2] // 2]
    ...
    with torch.no_grad(), torch.cuda.amp.autocast():
        for idx, indice in tqdm(enumerate(range(0, len(prompt_list_rank), args.bs)), desc='Sample Batch'):
            ...
            for sample_idx in range(args.n_samples):
                batch_images = image_guided_synthesis(
                ...
                # Save immediately, free memory
                for nn in range(batch_images.shape[0]):
                    save_results_seperate(prompts[nn], batch_images[nn:nn + 1], filenames[nn], fakedir, fps=8,
                                          loop=args.loop)
